--[[
    ...
    Universal entry point. Auto-loads mm2.lua when in MM2.
...
-- ── Universal loops  (run in every game) ───────────────
task.spawn(function()
    while true do
...
local StatusSection = MM2Tab:Section({ Title = "🎮 Status", Icon = "info", Opened = true })
if isInMM2 then
    StatusSection:Paragraph({
        Title   = "✅ Murder Mystery 2 detected!",
        Content = "MM2 features will load automatically in 3 seconds.",
    })
else
    StatusSection:Paragraph({
        Title   = "⚠️ Not in Murder Mystery 2",
        Content = "PlaceId: "..tostring(game.PlaceId).."  |  Required: "..tostring(MM2_PLACE_ID),
    })
    StatusSection:Paragraph({
        Title   = "MM2 features are locked.",
        Content = "Join Murder Mystery 2 to unlock all features.",
    })
end

local ScriptSection = MM2Tab:Section({ Title = "📦 Script", Icon = "package", Opened = true })
...
local function loadMM2()
    ...
    if not okH then
        ...
        ScriptSection:Paragraph({ Title = "❌ Download failed.", Content = tostring(src):sub(1,80) })
    ...
    if okE then
        ...
        ScriptSection:Paragraph({ Title = "❌ Execution error.", Content = tostring(err):sub(1,80) })
        mm2Attempted = false
    end
end
