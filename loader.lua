...
if getgenv().QuantumX_Loader_Loaded then return end
...
getgenv().QuantumX_Config = { speedEnabled = false, speedValue = 16, noclipEnabled = false }
...
local function safeHttp(url)
    local ok, r = pcall(game.HttpGet, game, url); return ok, r
end
local function safeExec(src)
    ...
    local ok2, e = pcall(fn); return ok2, tostring(e)
end

-- Universal Speed loop
task.spawn(function()
    while true do
        if getgenv().QuantumX_Config.speedEnabled then
            local h = getHumanoid(); if h then h.WalkSpeed = getgenv().QuantumX_Config.speedValue end
        end
        task.wait()
    end
end)

-- Universal Noclip loop
...
for i, url in ipairs(WINDUI_URLS) do
    print(("[QuantumX] WindUI attempt %d/%d …"):format(i, #WINDUI_URLS))
    local okH, src = safeHttp(url)
    if not okH or type(src) ~= "string" or #src < 100 then warn("[QuantumX] HTTP fail #"..i); continue end
    local okC, fn = pcall(loadstring, src)
    if not okC or type(fn) ~= "function" then warn("[QuantumX] Compile fail #"..i); continue end
    local okE, lib = pcall(fn)
    if not okE or lib == nil then warn("[QuantumX] Exec fail #"..i); continue end
    WindUI = lib; print("[QuantumX] ✅ WindUI loaded."); break
end

if not WindUI then warn("[QuantumX] ❌ WindUI failed – Speed/Noclip still active."); return end
...
do
    local ok, r = pcall(function()
        return WindUI:CreateWindow({
            Title = "Quantum X", SubTitle = DISCORD, Author = DISCORD,
    ...
    if not ok or r == nil then warn("[QuantumX] ❌ Window failed: "..tostring(r)); return end
    Window = r; getgenv().QuantumX_Window = Window
    print("[QuantumX] ✅ Window ready.")
end
...
local MS = UTab:Section({ Title = "⚡ Movement", Icon = "zap", Opened = true })
MS:Toggle({ Title = "Speed Hack", Default = false, Callback = function(v)
    getgenv().QuantumX_Config.speedEnabled = v
    if getgenv().MM2Config then getgenv().MM2Config.speedEnabled = v end
end })
MS:Slider({ Title = "Walk Speed", Min = 16, Max = 350, Default = 16, Callback = function(v)
    getgenv().QuantumX_Config.speedValue = v
    if getgenv().MM2Config then getgenv().MM2Config.speedValue = v end
end })
MS:Toggle({ Title = "Noclip", Default = false, Callback = function(v)
    getgenv().QuantumX_Config.noclipEnabled = v
    if getgenv().MM2Config then getgenv().MM2Config.noclipEnabled = v end
end })

local MiscS = UTab:Section({ Title = "🛠️ Misc", Icon = "settings", Opened = true })
MiscS:Button({ Title = "Rejoin Server", Callback = function()
    pcall(function() game:GetService("TeleportService"):Teleport(game.PlaceId, lp) end)
end })

local DevS = UTab:Section({ Title = "🔧 Dev Tools", Icon = "terminal", Opened = false })
DevS:Button({ Title = "Infinite Yield", Callback = function() local ok,s = safeHttp("https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/source"); if ok then safeExec(s) end end })
DevS:Button({ Title = "Dex Explorer",   Callback = function() local ok,s = safeHttp("https://raw.githubusercontent.com/infyiff/backup/main/dex.lua"); if ok then safeExec(s) end end })
DevS:Button({ Title = "SimpleSpy",      Callback = function() local ok,s = safeHttp("https://raw.githubusercontent.com/78n/SimpleSpy/main/SimpleSpyBeta.lua"); if ok then safeExec(s) end end })

-- Silent auto-load mm2.lua when in MM2
    ...
    task.delay(2, function()
        if not getgenv().QuantumX_MM2_Loaded then
            ...
            if okH then
                ...
                if not okE then warn("[QuantumX] mm2.lua error: "..err) end
            else
                warn("[QuantumX] mm2.lua download failed: "..tostring(src))
            end
        end
    end)
end

print("[QuantumX] ✅ Loader "..VERSION.." ready.")
